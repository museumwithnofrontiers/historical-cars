...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Historical Cars")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/cce9ed43-6802-51d5-8553-802e6dafb35e')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/a3a7e6de-6cff-51a1-9392-ab310f7efdc7')
    ...
    const isl = await mountSite('#/item/cce9ed43-6802-51d5-8553-802e6dafb35e')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === 'cce9ed43-6802-51d5-8553-802e6dafb35e')
  ...
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    ...
    const item = items.find((i) => i.project_id === islProjectId) ?? { internal_name: "Siever's patent pedestrian carriage for Ladies and Gentlemen" }
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/5aea6a37-19f3-5c61-ac76-f38f3922c376')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain("Romania")
    expect(host.textContent).toContain('National Museum of Romanian History')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/8cde1ffe-e1cc-52a8-9261-ee16bcaac48f')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('National Museum of Romanian History'), { timeout: 20000 })
    await vi.waitFor(() => expect(host.textContent).toContain("Bucharest"), { timeout: 20000 })
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/8cde1ffe-e1cc-52a8-9261-ee16bcaac48f/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['historicalCars', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
